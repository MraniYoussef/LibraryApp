...
public class LibraryApplication implements CommandLineRunner {
	...
	public static void main(String[] args) {
	  ...
	  corsConfiguration.setAllowedOrigins(Arrays.asList("http://localhost:4200", "http://localhost:3000", "http://localhost:3001", "http://localhost:3002"));
	...
	public void run(String... args) throws Exception  {
		...
		/*
		 * subscriptionService.createSubscription(55L); List<BookReservation>
		 * listBookReservationToBlock =
		 * bookReservationService.findAllBookReservations();
		 * 
		 * for (BookReservation list : listBookReservationToBlock) { Date dateNow = new
		 * Date(); //si le délai est dépassé
		 * if(list.getDateTermination().compareTo(dateNow)< 0) { Long idMember =
		 * list.getMember().getIdMember();
		 * bookReservationService.blockLateMembers(dateNow); }
		 * 
		 * } System.out.println("voila le book"); List<Book> book =
		 * bookService.findBookByTitle("Dreamm"); System.out.println(book);
		 */
		 ...
		 * brs1=null; Book bookToSave = new Book("aut1", "Illusions", bt1.ROMAN, 200,
		 * 20); Book book = bookRepository.save(bookToSave); Subscription sb1=null;
		 * Employee A1= employeeRepository.save(new Employee("Abdellah", "Abdellah",
		 * "1234", "A1", "Abdellah@gmail.com")); Member memeber=
		 * memberRepsitory.save(new Member( "ahmed","ah","11","a5","a@g","56",new
		 * Date(),mf1,sb1 ));
		 * 
		 * 
